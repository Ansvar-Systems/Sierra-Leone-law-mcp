/**
 * search_eu_implementations — Search EU directives/regulations that have Sierra Leonean implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_sierra leonean_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  sierra leonean_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as sierra leonean_statute_count
  ...
  if (input.has_sierra leonean_implementation) {
    sql += ' HAVING sierra leonean_statute_count > 0';
  }
