/**
 * get_sierra leonean_implementations — Find Sierra Leonean statutes implementing a specific EU directive/regulation
...
export interface GetSierra LeoneanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface Sierra LeoneanImplementationResult {
  document_id: string;
...
export async function getSierra LeoneanImplementations(
  db: InstanceType<typeof Database>,
  input: GetSierra LeoneanImplementationsInput,
): Promise<ToolResponse<Sierra LeoneanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as Sierra LeoneanImplementationResult[];
