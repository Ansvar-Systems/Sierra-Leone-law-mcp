#!/usr/bin/env tsx
/**
 * Sierra Leone Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawsierra leonean.com. Sierra Leonean proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawsierra leonean.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'sierra leonean-law-mcp/1.0 (https://github.com/Ansvar-Systems/sierra leonean-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Sierra Leone Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawsierra leonean.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawsierra leonean.com (Federal Negarit Gazette PDFs)`);
