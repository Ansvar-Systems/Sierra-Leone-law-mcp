#!/usr/bin/env tsx
/**
 * Sierra Leonean Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawsierra leonean.com';
const PORTAL_NAME = 'Sierra Leone Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/sierra leonean-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Sierra Leonean Law MCP — Data Freshness Check');
