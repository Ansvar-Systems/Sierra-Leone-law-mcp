#!/usr/bin/env tsx
/**
 * Drift detection for Sierra Leone Law MCP.
 *
 * Checks if upstream Sierra Leone Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Sierra Leone Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'sierra leonean-law-mcp/1.0 drift-detect' },
      });
